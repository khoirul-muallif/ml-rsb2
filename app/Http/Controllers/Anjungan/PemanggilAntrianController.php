...
class PemanggilAntrianController extends Controller
{
    ...
    private function renderPanggil(Request $request, $type)
    {
        ...
        //  AMBIL LOKET DARI PARAMETER URL
        $currentLoket = $request->get('loket', '1');
        
        // Handle form lompat antrian
        if ($request->has('antrian') && $request->filled('antrian')) {
            $this->lompatAntrian($type, $request->antrian, $currentLoket); // ← currentLoket harus dikirim
            return redirect($request->url() . '?show=' . $config['show'] . '&loket=' . $currentLoket . '&skip_audio=1');
        }
        
        //  Handle tombol "Berikutnya"
        if ($request->get('action') === 'next') {
           // Log::info('🔵 BERIKUTNYA (SILENT)', ['type' => $type, 'loket' => $currentLoket]);
            
            $this->panggilAntrianBerikutnya($type, $currentLoket, true); // ← currentLoket harus dikirim
    ...
    private function panggilAntrianBerikutnya($type, $loket, $silent = true)
    {
        ...
        // Log::info($silent ? '🔇 SILENT' : '🔊 WITH AUDIO', [
        //     'nomor' => $nomorBerikutnya,
        //     'loket' => $loket  // ← PENTING: Log untuk debug
        // ]);
        
        // Update database antrian_loket
        AntrianLoket::where('type', $type)
            ->where('noantrian', $nomorBerikutnya)
            ->whereDate('postdate', today())
            ->update([
                'end_time' => now()->format('H:i:s'),
                'loket' => $loket,  // ← Simpan ke field loket
                'status' => '1'
            ]);
        
        // ✅ PENTING: Update settings dengan loket yang dipilih
        MliteSetting::setSetting('anjungan', "panggil_{$fieldPrefix}", $loket);  // ← INI YANG PENTING!
        MliteSetting::setSetting('anjungan', "panggil_{$fieldPrefix}_nomor", $nomorBerikutnya);
        
        // Log::info('✅ Settings updated', [
        //     "panggil_{$fieldPrefix}" => $loket,
        //     "panggil_{$fieldPrefix}_nomor" => $nomorBerikutnya
        // ]);
    }
    ...
    private function lompatAntrian($type, $nomor, $loket, $silent = true)
    {
        ...
        AntrianLoket::where('type', $type)
            ...
            ->update([
                'end_time' => now()->format('H:i:s'),
                'loket' => $loket,  // ← Simpan loket
                'status' => '1'
            ]);
        
        //  PENTING: Update settings
        MliteSetting::setSetting('anjungan', "panggil_{$fieldPrefix}", $loket);  // ← INI YANG PENTING!
    ...
    public function setPanggil(Request $request)
    {
        ...
        $loket = $validated['loket'] ?? '1';  // ← Default 1 jika tidak ada
        
        // Log::info('🔵 API setPanggil (WITH AUDIO)', [
        //     'type' => $type,
        //     'nomor' => $noantrian,
        //     'loket' => $loket  // ← PENTING
        // ]);
