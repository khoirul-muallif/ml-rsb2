...
@push('styles')
    ...
    <style>
        ...
        .loket-btn.active {
            background: linear-gradient(135deg, {{ $config['color']['from'] }} 0%, {{ $config['color']['to'] }} 100%);
            border-color: {{ $config['color']['from'] }};
            color: {{ $config['color']['text'] ?? '#fff' }};
        ...
        .current-info {
            text-align: center;
            padding: 15px;
            background: rgba({{ $config['color']['from'] ?? '102, 126, 234' }}, 0.1);
            border-radius: 8px;
            color: #333;
            font-size: 1rem;
        }
...
<div class="container-pemanggil">
    ...
    {{-- ✅ NEW: LOKET SELECTOR --}}
    @if(count($loket) > 1)
    <div class="loket-selector-card">
        <div class="selector-header">
            <i class="fa fa-door-open"></i> Pilih Loket/Counter
        </div>
        <div class="loket-buttons">
            @foreach($loket as $num)
                <a href="{{ route('anjungan.pemanggil') }}?show={{ $show }}&loket={{ $num }}" 
                   class="loket-btn {{ $current_loket == $num ? 'active' : '' }}"
                   title="Switch ke Loket {{ $num }}">
                    @if($current_loket == $num)
        ...
        <div class="current-info">
            ...
            Anda sedang mengoperasikan: <strong>{{ $config['full_label'] }} - Loket {{ $current_loket }}</strong>
        </div>
    </div>
    @endif
    ...
    <x-anjungan.instructions 
        ...
        :items="[
            ...
            ['icon' => 'fa-door-open', 'title' => 'Switch Loket', 'text' => 'Klik tombol loket untuk berpindah counter'],
